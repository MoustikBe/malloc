#include <stdio.h>
#include <string.h>
#include <stdlib.h>
#include <time.h>
/* lib */
#include "libft/libft.h"
/* lib */
#include "includes/malloc.h"

/* LD_PRELOAD=./libft_malloc.so [program] */

static void print_header(const char *title)
{
    ft_printf("\n========== %s ==========\n", title);
}

static void check_memory_content(const char *msg, void *p, const char *expected, size_t size)
{
    if (memcmp(p, expected, size) == 0)
        ft_printf("[OK] %s\n", msg);
    else
        ft_printf("[ERROR] %s: contenu incorrect\n", msg);
}

int main(void)
{
    /* Libft test */
    char **new = ft_split("Hello comment ca va ?", ' ');
    for(int i = 0; new[i]; i++)
        ft_printf("new -> %s\n", new[i]);
    /* Different allocation test */
    srand((unsigned int)time(NULL));

    print_header("Test basique d'allocation et libération");

    char *str = (char *)malloc(20);
    if (!str)
        ft_printf("[ERROR] malloc a renvoyé NULL\n");
    else {
        strcpy(str, "Hello world!");
        ft_printf("Contenu: %s\n", str);
        free(str);
        ft_printf("[OK] Allocation + free simples\n");
    }

    print_header("Test de multiples allocations");

    void *ptrs[10];
    for (int i = 0; i < 10; i++) {
        ptrs[i] = malloc(50 + i * 10);
        if (!ptrs[i]) {
            ft_printf("[ERROR] Allocation %d a échoué\n", i);
            return 1;
        }
        memset(ptrs[i], i, 50 + i * 10);
    }
    ft_printf("[OK] 10 allocations successives\n");

    for (int i = 0; i < 10; i += 2)
        free(ptrs[i]);
    ft_printf("[OK] Libération de la moitié des blocs\n");

    for (int i = 1; i < 10; i += 2)
        free(ptrs[i]);
    ft_printf("[OK] Libération du reste\n");

    print_header("Test de réallocation");

    char *data = malloc(10);
    strcpy(data, "abcde");
    data = realloc(data, 20);
    strcat(data, "12345");
    ft_printf("Après realloc: %s\n", data);
    check_memory_content("Realloc conserve les données", data, "abcde12345", 10);
    free(data);

    print_header("Test de realloc sur NULL et taille = 0");

    char *null_realloc = realloc(NULL, 30);
    if (null_realloc)
        ft_printf("[OK] realloc(NULL, size) agit comme malloc\n");
    free(null_realloc);

    char *small = malloc(10);
    small = realloc(small, 0);
    if (small == NULL)
        ft_printf("[OK] realloc(ptr, 0) renvoie NULL et libère\n");

    print_header("Test de fragmentation et réutilisation de blocs");

    void *a = malloc(100);
    void *b = malloc(200);
    void *c = malloc(50);
    free(b);
    void *d = malloc(180);
    if (d)
        ft_printf("[OK] Bloc libre réutilisé (fragmentation test)\n");
    free(a);
    free(c);
    free(d);

    print_header("Test de stress aléatoire");

    void *pool[1000] = {0};
    for (int i = 0; i < 5000; i++) {
        int idx = rand() % 1000;
        if (pool[idx]) {
            free(pool[idx]);
            pool[idx] = NULL;
        } else {
            size_t size = (rand() % 256) + 1;
            pool[idx] = malloc(size);
            if (pool[idx])
                memset(pool[idx], 0xAA, size);
        }
    }
    for (int i = 0; i < 1000; i++)
        if (pool[i])
            free(pool[i]);
    ft_printf("[OK] Stress test terminé sans crash\n");

    print_header("Test double free (comportement attendu : sécurisé ou erreur contrôlée)");

    void *df = malloc(42);
    free(df);
    ft_printf("Libération une fois : OK\n");
    free(df);
    ft_printf("Tentative double free terminée\n");
    
    print_header("Test realloc sur pointeur invalide");

    int fake;
    void *invalid = &fake;
    void *res = realloc(invalid, 50);
    if (res)
        ft_printf("[WARNING] realloc a accepté un pointeur invalide !\n");
    else
        ft_printf("[OK] realloc a détecté un pointeur invalide\n");

    print_header("Tous les tests terminés ✅");

    return 0;
}
